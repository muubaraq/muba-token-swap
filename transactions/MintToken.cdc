...
transaction(receiver: Address, amount: UFix64) {
    ...
    prepare(signer: AuthAccount) {
        ...
        let minter = signer.borrow<&MubaToken.Minter>(from: /storage/MinterStorage)
            ?? panic("You are not an allowed MubaToken minter")
        
      
        let receiverVault = getAccount(receiver)
            .getCapability<&MubaToken.Vault{FungibleToken.Receiver}>(/public/Vault)
            .borrow()
            ?? panic("Error: Check your MubaToken Vault status")
