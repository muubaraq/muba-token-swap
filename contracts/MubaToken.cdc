...
pub contract MubaToken: FungibleToken {
    ...
    pub resource Vault: FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, CollectionPublic {
        ...
        pub fun deposit(from: @FungibleToken.Vault) {
            let vault <- from as! @MubaToken.Vault
        ...
        // Destroy the vault
        destroy() {
            MubaToken.totalSupply = MubaToken.totalSupply - self.balance
        }
    }
    ...
    pub resource Minter {
        ...
        pub fun mintToken(amount: UFix64): @FungibleToken.Vault {
            MubaToken.totalSupply = MubaToken.totalSupply + amount
            return <-create Vault(balance: amount)
        }
    }
    ...
    init() {
        ...
        self.account.link<&MubaToken.Minter>(/public/Minter, target: /storage/MinterStorage)
